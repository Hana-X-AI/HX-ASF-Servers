...
- governace/goals/, KDDs, `servers/SERVER-REGISTRY.md`, live host evidence.
  ...
  entry previously read "goals/, KDDs, `servers/SERVER-REGISTRY.md`, live host
  evidence." As part of the governance alignment batch the durable goal tree moved
  `goals/` -> `governace/goals/` (KDD-0002 Amendment 1); the former path is preserved
  here as history and `governace/goals/` is the effective path for existing goal
  records.]
