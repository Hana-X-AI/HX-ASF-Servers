...
| Execution model | Profile-briefed standalone `kimi` sessions bound to her assigned lane (`kimi -m omniroute/chat-x --agent-file agents/carol/profile.md`); the catalog is Carol's persistent state — idempotent re-ingestion, never memory [superseded 2026-08-28: originally "Profile-briefed sub-agent sessions" — the substrate exception was RETRACTED by owner directive: no sub-agent sessions on moonshot-ai. Original preserved here as history] |
| Model lane | OpenAI gpt-oss-120b (`openrouter/openai/gpt-oss-120b`, upstream provider AkashML, via OmniRoute hxs-8) — owner-assigned 2026-08-28, route probed live same day (model served exactly, reasoning stream coherent); schema and mechanical validation stay deterministic (`scripts/validate.py`); stop-and-escalate on backend failure, no substitution [superseded 2026-08-28: originally Chat-X (`ollama-local/hx-qwen3.5-9b-64k`, hxs-4) per KDD-0013 — superseded by owner directive the same day; the Chat-X gateway connection was already INACTIVE (posture-blocked) per state-log row 28; original preserved in KDD-0013's record as history] |
