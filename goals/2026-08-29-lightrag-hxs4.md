...
- Agent lane(s): **raphael** (LightRAG systems engineer, Family 3,
  ...
  via OmniRoute hxs-1). Meta-X LLM binding via OmniRoute. Embedding model
  (bge-m3) install on hxs-4 via Ollama requires john's lane (Ollama engineer).
...
- In scope:
  ...
  - LLM binding via OmniRoute (OpenAI-compatible endpoint on hxs-8)
...
- Constraints:
  ...
  - LLM binding must use local models via OmniRoute (local-model-first rule)
...
| SC-05 | LLM binding works | LightRAG config uses OmniRoute OpenAI-compatible endpoint | Query returns generated text | query output | governor |
...
| SC-11 | Credentials stored | All LightRAG credentials in `.local.env` | QDRANT_URL, QDRANT_API_KEY, LIGHTRAG_API_KEY, LLM_BINDING vars present | grep .local.env (no values printed) | governor |
...
- HITL checkpoints: bge-m3 install on hxs-4 (john's lane — cross-lane
  dependency); LLM model confirmation (Meta-X via OmniRoute); embedding dim
  verification (bge-m3 = 1024); pytest presence verification before SC-14/15
...
MCP clients → lightragmcp (Suryan v1.1.0, 30 tools, stdio transport)
