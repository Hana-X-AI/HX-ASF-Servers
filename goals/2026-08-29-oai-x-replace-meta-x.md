...
- In scope:
  - Pull gpt-oss-20b model into Ollama on hxs-3
  - Create Ollama Modelfile with call-sign `hx-oai-x-64k` (or owner-selected context window)
  - Verify model serves correctly via Ollama API
  - Verify model is accessible via OmniRoute (hxs-8 routing)
  - Update OmniRoute connection/routing if needed
  - Update all references: AGENTS.md, agents/README.md, KDD-0013, agent
    profiles (john, rick — both on Meta-X lane), system-mapping
  - Decommission Meta-X (remove from Ollama, update OmniRoute)
...
- Out of scope:
  ...
  - OmniRoute itself (trinity's lane — john only installs the model)
...
- Constraints:
  ...
  - OmniRoute routing must be updated to point to the new model
...
| SC-06 | Meta-X decommissioned | `ollama list \| grep muse-glimmer` (alias check) AND verify the frozen artifact is removed from the store (`ollama show muse-glimmer:30b` fails, or the artifact digest `de878ce3…` is no longer present in `/usr/share/ollama/.ollama/models/blobs` or `ollama list` no longer references it) | Not listed; artifact and alias removed from store | command output | governor |
...
- Rollback / containment: keep Meta-X until OAI-X passes SC-05; only
  decommission after full verification
...
- Model: gpt-oss-20b (OpenAI gpt-oss, 20B params, available on Ollama)
