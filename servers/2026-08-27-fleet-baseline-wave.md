...
- `fleet-verify-baseline.sh` ×12: **OVERALL hosts=12, hosts-with-FAIL=0** (hxs-1..4: 10 PASS each; server-default hosts: 1 PASS + 1 REPORT each — the report-level rule's actual now satisfies the declared direction on every host).
  - [correction 2026-08-27, labeled, append-only] **Standards revision for this run:** the sweep above ran against `fleet-standard.yaml` v0.1 (server-default: timezone enforce + ntp_server `contains time.cloudflare.com` at **report** level, no mask rules — hence "1 PASS + 1 REPORT" per server-default host). The file was not yet committed at run time, so its pre-amendment digest was not recorded; provenance anchor: the library manifest digest including the pre-amendment file was `0c4474fd…` (DOC-fleet-script-library, pre-wave). **Separate result against the CURRENT standard** (amended 2026-08-27 after rick's F-4: server-default ntp_enabled/ntp_synchronized/4-target-mask set added at enforce level, ntp_server lifted report→enforce; sha256 `4c298ea698abc202a7ce658ca534e1ee45ef8670de0b1000bcffd17d470137a4`; library manifest digest `879c9a7c…`): independent governor re-run 2026-08-27T20:2xZ — llm-host 10/10 ×4, server-default 8/8 ×8, OVERALL hosts=12, hosts-with-FAIL=0 (fleet pilot log row 3).
