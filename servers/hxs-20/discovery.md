...
## Identity
- Record identity: hxs-20
- Observed shell prompt: `hx-20`. A direct `hostname` or `hostnamectl` result was not supplied, so the on-host hostname remains to be verified
...
- Current memory usage: 1 percent in the Ubuntu system-information summary
...
- IPv4: 192.168.50.220; prefix length unavailable from supplied evidence
...
- Update state: 0 updates can be applied immediately. Expanded Security Maintenance for Applications is not enabled
- Reboot required: unavailable from supplied evidence
- Reported system temperature: 47.9 C
...
- Memory: installed capacity and DIMM topology unavailable; the system-information summary reported 1 percent in use
...
- Network: eno1 carries 192.168.50.220; link characteristics and remaining network configuration unavailable
- Constraints / notable characteristics:
  - host identity is not closed because the owner-selected record name `hxs-20` and observed shell prompt `hx-20` differ
...
## Notes
- Evidence timestamp: 2026-08-28 00:24:04 UTC
- Last-login line reported 2026-08-28 00:21:10 UTC from 192.168.50.115
- The Ubuntu summary reported system load 0.07, 144 processes and 0 users logged in
