...
2. The assigned agent writes or refines the goal file from
   `governace/goals/_template.md` (the `/write-goal` pattern: a rough intention
   becomes a completion contract).
...
- One file per goal: `governace/goals/<YYYY-MM-DD>-<short-slug>.md`; the file name
  is a date-slug Goal ID (e.g. `2026-08-24-hx1-ollama-qwen38-27b`) — not a decision
  identifier like KDD-0002 (KDD-0002 is the decision that designates this
  convention, not a Goal ID).
- Status values: `draft`, `approved`, `in-progress`, `blocked`, `done`, `abandoned`
- `governace/goals/` is the designated durable work-state system; GitHub Projects
  is deferred (KDD-0002).
