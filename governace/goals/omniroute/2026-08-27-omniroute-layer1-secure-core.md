...
- OD-13: env-provision `JWT_SECRET` + `API_KEY_SECRET` (generated locally, provisioned via the service environment, never accepted from product defaults) **and** set `STORAGE_ENCRYPTION_KEY` (activates the AES-256-GCM field encryption — the product ships plaintext-passthrough without it). [amendment 2026-08-27 — approved injection mechanism and prohibited locations, matching the deployed L1-M2 reality: values are generated on-host and injected ONLY via a root-only 0640 systemd drop-in, recorded by sha256 never value. PROHIBITED at rest: repository files, world-readable unit files, logs/journals/receipts/artifacts, and the database (secrets namespace must hold 0 plaintext rows; connection fields must be `enc:v1:` ciphertext). ALLOWED: plaintext in process memory and the service's own runtime environment — that is inherent to env injection and is not a violation.] [labeled clarification 2026-08-30, append-only: the secret-bearing systemd drop-in must be owned **root:root** with **0640** permissions so no non-root group can read it — 0640 grants group-read, so the owning group must be root; where only the system manager (root) needs access, 0600 is the acceptable stricter alternative. Verify ownership and mode on the deployed drop-in.]
...
3. Routing: all four backends registered with identity evidence; **direct-vs-routed parity** — a known-answer task through each backend direct vs routed through OmniRoute, with response-shape comparison and usage accounting visible in the ledger's own evidence paths. [amendment 2026-08-27: hxs-4/Chat-X registration and identity evidence remain MANDATORY, but its parity check is explicitly WAIVED as posture-blocked (approved posture exception per OD-08 amendment, not a failure) while its loopback-only posture stands; the waiver is recorded in the acceptance evidence. Parity stays required for every reachable backend.]
