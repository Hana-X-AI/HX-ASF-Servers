...
[LABELED CORRECTION 2026-08-30, append-only — DEPLOYED ARTIFACT: the exact
model above (mannix/qwen3.6-27b-a3b-coderx:vision-Q4_K_M, owner D3) was
SUPERSEDED by the owner's deployment decision to **Qwen2.5-Coder-32B, AWQ Int4,
TP=2** (deployed and verified per SERVER-REGISTRY, status READY; see the
labeled D3 note 2026-08-29). The intent and acceptance contract read as
referencing the deployed Qwen2.5-Coder-32B AWQ Int4 artifact; the original
artifact wording above is preserved as history.]
...
- D3: model = `mannix/qwen3.6-27b-a3b-coderx:vision-Q4_K_M` (CoderX, top-8,
  ...
  [LABELED CORRECTION 2026-08-30, append-only — DEPLOYED ARTIFACT: D3's model
  was SUPERSEDED by the owner's deployment decision to **Qwen2.5-Coder-32B, AWQ
  Int4, TP=2** (deployed and verified per SERVER-REGISTRY, status READY). The
  original D3 artifact above is preserved as history; the operative artifact is
  Qwen2.5-Coder-32B AWQ Int4.]
...
| --- | --- | --- | --- | --- | --- |
| SC-01 | Model identity frozen | explicit tag + full local digest, capabilities, projector, template, license | Exact approved artifact recorded (Qwen2.5-Coder-32B, AWQ Int4, TP=2 — deployed per SERVER-REGISTRY; original vision-Q4_K_M requirement preserved as history in the D3/Intent corrections) | model evidence, ollama show | KK3 gate |
