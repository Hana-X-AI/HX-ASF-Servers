...
Source of record (intake): `governace/sponsor/hxs-1/2026-08-31-factory-intake-ollama-fleet-requalification-wo1-hxs1.md`
...
- D5 — Web search / Ollama cloud: ON and active (fleet-wide posture); test and verify on hxs-1 only. Cloud-MODEL substitution remains prohibited.
...
Append-only. Scope-lock APPROVED 2026-08-31: owner decisions D1-D5 answered
(service FQDN `qwen-x.hx.local.arpa`; auto-update; three full reboots + one
performance run, no 24-hour soak; full current-RAG-pipeline access with no new
systems; web search ON). No operational status change or acceptance has been
recorded yet.
