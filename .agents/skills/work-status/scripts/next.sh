...
# Usage: bash .agents/skills/work-status/scripts/next.sh
set -u
...
if [ "$found" -eq 0 ]; then
  ...
  echo "  • Check blocked: bash .agents/skills/work-status/scripts/blocked.sh"
  echo "  • Check in progress: bash .agents/skills/work-status/scripts/in-progress.sh"
else
